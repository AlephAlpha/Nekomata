...
* 在两个字符的组合中，排前六的分别是 `ᶦ{`、`ᵖ{`、`ʷ{`、`ˡ{`、`ᶠ{`、`ᵑ{`。也许可以像 Vyxal 等语言一样，让这些助词自动开启 block，省去一个 `{`。不过另外一些助词通常只修饰单个 built-in，自动开启 block 反而会额外需要一个 `}`。尤其是 `ᵖ`、`ᵑ`、`ᵐ`，两种用法都很常见，不好取舍。详见 [`particles.txt`](particles.txt)，以及下文的[关于助词](#关于助词)。
...
这里说的 ragged-list 是指可能有多层嵌套的列表，而且每个元素的长度、层数都不一定相同的列表。比如说 `[1, [2, 3], [[4], 5]]`。相关的 code golf 题目通常需要递归地对每一层列表进行某些操作，而不是只处理最外层。
...
- 像 `\iterate`、`\while`、`\nTimes` 这类，把修饰的函数重复作用在一个值上，直到满足某个条件。它们只能用于 arity 为 `n -> n` 的函数，也就是说输入和输出的个数必须相同。这个限制不太好改。一个可能的思路是，如果输入的函数的 arity 是 `m -> n`，`m != n`，则想办法把它转换成 `m -> m` 或 `n -> n` 的函数。具体怎么转换还不好说，但不管怎么转换，都会严重影响可读性。也许还是不要改了。
  ...
  - [ ] `m < n` 时暂时没想到什么好的方案。也许可以考虑在每一步迭代后弹出多余的 `n - m` 个元素。这对于 `\iterate` 来说可能会比较有用。
- 其它不好归类的助词，具体情况具体分析。
...
- [x] 重写现有的 Tutorial。考虑分成多个文件，每个文件只讲一个主题。
- [ ] REPL 的 `\Info` 命令，可以考虑改成能一次输出多个函数的信息。

试了一下把 Tutorial 完全交给 AI 来写，感觉还不错，几分钟就写出来了，粗略看了一下也没发现明显的错误，可读性还比我原来写的好很多。以后可以多用 AI。
...
用 Rust 重写整个项目……当然也只是幻想一下。其实解释器这种东西用 Haskell 写还是远比 Rust 要方便的。
